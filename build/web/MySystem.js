...
class MySystem {
    ...
    dispWebPage(pageName) {
        ...
        gr.mdSystem = new Model("mdSystem", "Md_system", {zIndex: 1000}, {});
        gr.mdSystem.build();
    ...
    baseTimer() {
        ...
        self.repaint(0);
        if (gr.mdMain) {
            if (gr.mdMain.mdClass.chkWatch)
                gr.mdMain.mdClass.chkWatch();
            gr.mdMain.chkWatch();
        }
        if (gr.mdSystem)
    ...
    setWatch(self, optsName, value) {
        ...
        for (; ; ) {
            if (optsName === "innerText")
                break;
            if (optsName === "innerTextColor")
                break;
            if (optsName === "baseColor")
                break;
            self.watch["_sysReDraw_f"] = 1;
            break;
        }
        ...
        if (value !== undefined) {
            var strA = optsName.split("#");
            if (strA.length === 2) {
                var cnt = KvLib.toInt(strA[1], null);
                if (cnt === null)
                    return;
                self.opts[optsName][cnt] = value;
                return;
            }
    ...
    setInputWatch(self, type, regName, optName, redraw_f) {
        if (!self)
            return;
        var ipObj = {};
        ipObj.cnt = 0;
        ...
        self.opts.inputRegs.push(ipObj);
    }
    ...
    inputWatch(self) {
        ...
        */
        ...
        for (var i = 0; i < self.opts.inputRegs.length; i++) {
            ...
            if (ipObj.type === "directName") {
                if (ipObj.inputName === "self.fatherMd.popOwner.stas.exStatus[\"1011\"][\"color\"]") {
                    //var obj = self.fatherMd.popOwner.stas.exStatus["1011"]["color"];
                    //var uu = 0;
                }
                var value;
                var str = "value=" + ipObj.inputName;
            ...
            if (ipObj.type === "fatherStas") {
                ...
                if (strA.length === 2) {
                    ...
                    value = self.fatherMd.stas[strA[0]][inx];
                    if (self.opts[ipObj.optName][inx] !== value) {
                        sys.setWatch(self, ipObj.optName, value);
                    }
                }
                continue;
    ...
    audioPlayBox(iobj) {
        ...
        if (!iobj.buttons)
            iobj.buttons = ["ESC"];
        var opts = {};
        opts.fileName = iobj.fileName;
        opts.buttons = iobj.buttons;
        var strA = iobj.fileName.split("/");
        opts.title = strA[strA.length - 1];
        opts.titleColor = "#ccf";
        opts.actionFunc = iobj.actionFunc;
        opts.playerWidth = iobj.width - 40;
        var mod = new Model("", "Md_audioPlayBox~sys", opts, {});
        var opts = {};
        opts.kvObj = mod;
        opts.w = iobj.width;
        opts.h = iobj.height;

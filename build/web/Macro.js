...
class Macro {
    ...
    getSaveType(modelSet, templateSet) {
        var optsSet = us.set.optsSet;
        var userSetNameType = modelSet + "~" + templateSet + "~typeSet";
        var typeSet = "sys";
        if (optsSet[userSetNameType]) {
            typeSet = optsSet[userSetNameType];
            if (!optsSet[modelSet + "~" + templateSet][typeSet])
                typeSet = "sys";
        }
        return typeSet;
    }

    uploadeFiles(actionFunc, accept) {
    ...
    setFootBar(layouts, lyMap, comps, cname) {
        ...
        comps[cname] = {name: "status1", type: "label~message", opts: opts};
        //========================
        var cname = lyMap.get("footBar_2");
        var opts = {};
        opts.innerText = "status2";
        comps[cname] = {name: "status2", type: "label~message", opts: opts};
        //========================
        var cname = lyMap.get("footBar_3");
        var opts = {};
        opts.innerText = "status3";
        comps[cname] = {name: "status3", type: "label~message", opts: opts};
    }
    
     
    selectBox(opts, w, h, maskTouchOff_f) {
        var mod = new Model("selectBox", "Md_selectBox~sys", opts, {});
        var popOpts = {};
        popOpts.kvObj = mod;
        popOpts.w = w;
        popOpts.h = h;
        popOpts.maskTouchOff_f = maskTouchOff_f;
        return sys.popOnModel(popOpts);
    }
    numpad(setObj, func) {
        var op = {};
        op.title = setObj.name;
        op.setObj = setObj;
        op.actionFunc = func;
        var mod = new Model("numpad", "Md_numpad~sys", op, {});
        var popOpts = {};
        popOpts.kvObj = mod;
        popOpts.w = 600;
        popOpts.h = 500;
        popOpts.maskTouchOff_f = 1;
        sys.popOnModel(popOpts);
    }

    keyboard(setObj, func) {
        var op = {};
        op.actionFunc = func;
        op.title = setObj.name;
        op.setObj = JSON.parse(JSON.stringify(setObj));
        if (setObj.dataType === "password") {
            op.setObj.dataType = "password";
            op.setObj.setType = "inputPassword";
        } else {
            op.setObj.dataType = "str";
            op.setObj.setType = "inputText";
        }
        var mod = new Model("keyboard", "Md_keyboard~sys", op, {});
        var popOpts = {};
        popOpts.kvObj = mod;
        popOpts.w = 0;
        popOpts.h = 500;
    ...
    inputPad(setObj, func) {
        ...
        var mod = new Model("inputPad", "Md_inputPad~sys", op, {});
        var popOpts = {};
        popOpts.kvObj = mod;
        popOpts.w = 800;
        popOpts.h = 500;
...
class KvBox {
    ...
    viewBox(opts, w, h) {
        var mod = new Model("viewBox", "Md_viewBox~sys", opts, {});
        var popOpts = {};
        popOpts.kvObj = mod;
        popOpts.w = w;
        popOpts.h = h;

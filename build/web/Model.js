...
class Md_audioPlayBox {
    static init() {
        var bobj = gr.modelOpts["Md_audioPlayBox"] = {};
        var dsc = bobj["optsDsc"] = {};
        var sobj = bobj["subOpts"] = {};
        bobj.propertyWidth = 600;
        bobj.propertyHeight = 150;
        ...
        bobj.titleHeight = 50;
        dsc.titleHeight = sys.setOptsSetFix("titleHeight", "nature999");
        bobj.buttonHeight = 50;
        dsc.buttonHeight = sys.setOptsSetFix("buttonHeight", "nature999");
        bobj.buttonWidth = 100;
        dsc.buttonWidth = sys.setOptsSetFix("buttonWidth", "nature999");

        bobj.autoPlay_f = 1;
        bobj.playerWidth = 500;
    ...
    afterCreate() {
        var md = this.md;
        var op = md.opts;
        var player = md.compRefs["audioPlayer"];
        var elem = player.elems["base"];
        var str = "<audio controls";
        if (op.autoPlay_f)
            str += " autoplay";
        str += " style='width: " + op.playerWidth + "px;'";
        str += ">";
        str += "<source src=" + op.fileName + " type='audio/mpeg'>";
    ...
    build(md) {
        ...
        //==================================
        var cname = lyMap.get("main") + "~" + 0;
        var opts = {};
        opts.innerTextColor = "#ccc";
        comps[cname] = {name: "audioPlayer", type: "plate~none", opts: opts};
    }
}
    ...
    build(md) {
        ...
        var actionFunc = function (iobj) {
            ...
            if (strA[0] === "mdEditOptsLine") {
                ...
                if(iobj.valueChange){
                    if(md.opts.valueChangeFunc){
                        md.opts.valueChangeFunc(iobj);
                    }
                }
...
class Md_filePicker {
    ...
    build(md) {
        ...
        var menuFunc = function (iobj) {
            ...
            if (itemId === "deleteFile") {
                var deleteFileFunc = function (iobj) {
                    if (iobj.buttonName !== "YES")
                        return;

                    var callBackFunc = function (iobj) {
                        if (iobj.length <= 0)
                            return;
                        var mdList = md.modelRefs['fileList'];
                        var scrollTop = mdList.mdClass.getScroll();
                        ...
                        var fileNameObj = md.compRefs["fileName"];
                        sys.setReDraw(fileNameObj, "innerText", "");
                    ...
                    var fileName = md.compRefs['fileName'].opts.innerText;
                    var strA = fileName.split("/");
                    gr.serverCallBack = callBackFunc;
                    Test.server_deleteFilesInDir("response error", "exeCallBackFunc", strA[0], [strA[1]]);
                    return;
                ...
                if (!fileName)
                    return;
                sys.mesBox("cy~warn", 500, "Delete this file ?", ["NO", "YES"], deleteFileFunc);
                return;
            }
        ...
        kexts.push(new Kext("deleteFile", "deleteFile", "", {enHint: "delete file"}));
        opts.menuKexts["rootMenu"] = kexts;
...
class Md_selectBox {
    ...
    afterCreate() {
        ...
        var op = self.md.opts;
        if (op.actionFunc)
            op.actionFunc({act: "reDraw",kvObj:self.md});
    }
    build(md) {
        var self = this;
        this.md = md;

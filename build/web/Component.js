...
class Component {
    ...
    chkWatch() {
        ...
        if (self.watch["baseColor"]) {
            var elem = self.elems["base"];
            elem.style.backgroundColor = self.opts.baseColor;
            delete self.watch["baseColor"];
        }
        self.watch = {};
        return;
    }
    ...
    create(fhid, x, y, w, h) {
        ...
        if ("setBaseElem") {
            ...
            if (op.hidden_f)
                elem.style.visibility = "hidden";
            //=====================================
            var lines = st.innerText.text.split("<br>").length;
            if(!op.writingMode)
            ...
            elem.innerHTML = st.innerText.text;
            var kvd = {};
            kvd.kvObj = self;
            elem.kvd = kvd;
        ...
        if ("setObject") {
            ...
            if (this.baseType === "input") {
                ...
                selem.id = this.kid + "_" + this.type;
                //selem.autocomplete="new-password";
                selem.autocomplete="none";
                selem.name=this.kid+Math.floor(Date.now()/1000);//disable auto fill
                ...
                if (op.pattern !== undefined && op.pattern !== null)
                 ...
                 if(this.subType==="password"){   
                    selem.type = "text";
                    selem.style["-webkit-text-security"]="disc";
                }
                else{
                    elem.type = this.subType;
                }
